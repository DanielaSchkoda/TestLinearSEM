...
1.  Generate data\
    First, we simulate data to fulfill the null hypothesis with number of nodes `p=2` and sample size `n=1000`.
...
```{r}
indep_stat(data, equality_constraints = all_minors_M, ineq_constraints = list(strassen_ineq))$PVAL
```
...
```{r}
indep_stat(data, equality_constraints = append(all_minors_M, list(Aronhold_invariant)))$PVAL
```
...
A_T <- construct_A_T(p)
a <- floor((p-1)/2)
rank_A_T <- p * choose(p-1, a)

data <- generate_data(n, p, l, "gamma", "H_0")

combine_pvalues(CRT(data, M, p)$PVAL, CRT(data, A_T, rank_A_T)$PVAL)
```
